...
struct LoadDataCommand: AsyncCommand {
    ...
    func run(using context: CommandContext, signature: Signature) async throws {
        ...
        let sourceURL = URL(fileURLWithPath: currentWorkingPath)
            .appending(path:"tmdb_dump_2025-04-10.zip")
        
        var destinationURL = URL(fileURLWithPath: currentWorkingPath)
        
        do {
            let res = try await runShellCommand("unzip tmdb_dump_2025-04-10.zip")
        ...
        // Open file
        let extractedPath = URL(fileURLWithPath: currentWorkingPath).appending(path:"tmdb_dump_2025-04-10.json")
